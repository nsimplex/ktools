...
#	error Unsupported native endianness.
	...
	public:
		IOHelper() : source_endianness(ktools_UNKNOWN_ENDIAN), target_endianness(ktools_LITTLE_ENDIAN) {}
		...
		bool hasInverseSourceOf(const IOHelper& h) const {
		...
		void copySource(const IOHelper& h) {
			source_endianness = h.source_endianness;
		}
		void copyTarget(const IOHelper& h) {
